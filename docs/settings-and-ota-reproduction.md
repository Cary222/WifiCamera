...
### 1. 云端 OTA 接口（默认 `http://139.196.14.90`）

当前默认使用 hjc 的个人阿里云服务器，临时给 C 重构版测试 OTA；HTTP 服务未设置登录密码。SSH 管理使用现有密钥，不是匿名登录。该服务用于版本查询和包下载测试，设备锁与注册目前是测试桩。

原公司 OTA 服务器为 `http://170.106.80.91:7788`。需要切换时设置 `EXPO_PUBLIC_OTA_BACKEND_URL`；App 运行时和构建配置均支持该覆盖。
