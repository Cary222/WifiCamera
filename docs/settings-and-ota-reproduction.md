...
当前默认使用公司 OTA 服务器 `http://170.106.80.91:7788`。

hjc 的个人阿里云 `http://139.196.14.90` 保留作 C 重构版临时 OTA 测试服务，HTTP 未设置登录密码；SSH 管理使用现有密钥。该服务用于版本查询和包下载测试，设备锁与注册目前是测试桩。需要使用时设置 `EXPO_PUBLIC_OTA_BACKEND_URL`；App 运行时和构建配置均支持该覆盖。
