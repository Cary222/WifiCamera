...
### P1：星图内多语言缺失（浮层已解决，引擎侧仍待做）

参考 `i18n` 下 `gui` / `sky` / `otypes` / `skycultures` 各 17–18 语言；我们只有 `skycultures-zh_Hans.qm`。

- 已完成（2026-09-08）：星图浮层文案（抽屉、指南针、设置、五个细分布局、天体卡、日历、观测工具、地景/位置弹窗）全部走 `deep_space.*` i18n，共 258 条 key，zh/en/ar 三语齐备；城市名、星座名、月份、Bortle 档位等数据名也随语言切换。引擎侧语言链路实测正确（`?lang=zh` / `?lang=en`，ar 回退 en）。
