...
| **#30 预览提示文字方向** | 断流/未连流状态下进入风景、星空、行星模式 | **「正在连接预览…」或失败提示文字直接脱离 90° 旋转容器，全模式统一保持水平正向居中显示，彻底消除竖排 90° 歪头问题。** | 代码重构与组件测试通过 |
| **#5 拍摄照片存储归属** | 检查拍摄与相册下载逻辑 | **确认既定产品架构：原片保存在板端 SD 卡，用户在相册中按需下载到手机本地系统相册，避免连拍瞬时挤占手机存储与带宽，状态已闭环。** | 逻辑对齐闭环 |
