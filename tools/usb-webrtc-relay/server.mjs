...
const relayAdvertiseHost = process.env.RELAY_WEBRTC_ADVERTISE_HOST || '10.0.2.2';
...
function proxyWhep(request, response) {
  ...
  const upstream = httpRequest({
    ...
    upstreamResponse.on('end', () => {
      const body = rewriteSdpCandidates(Buffer.concat(chunks).toString('utf8'));
...
server.listen(port, '0.0.0.0', () => {
  ...
  console.log(`[usb-relay] UDP ${relayBindHost}:${relayUdpPort}; candidate ${relayAdvertiseHost}:${relayUdpPort}`);
