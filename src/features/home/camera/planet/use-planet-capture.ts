...
/**
 * Sensor crops mirroring the board's ROI presets.
 */
export const PLANET_ROI_PRESETS: RoiPreset[] = [
  {
    ...
    label: '1920×1080',
    resolution: '1080P',
    descriptionKey: 'planet.roi_full_desc',
    fps: 30,
    x: 0,
    y: 0,
    width: 1920,
  ...
  },
  {
    ...
    label: '800×600',
    resolution: '800P',
    descriptionKey: 'planet.roi_medium_desc',
    fps: 60,
    x: 560,
    y: 240,
    width: 800,
  ...
  },
  {
    ...
    label: '640×480',
    resolution: '640P',
    descriptionKey: 'planet.roi_deep_desc',
    fps: 60,
    x: 640,
    y: 300,
    width: 640,
...
export function usePlanetCapture({
  ...
  const [roiSequence, setRoiSequence] = useState(0);
  const [recordingSeconds, setRecordingSeconds] = useState(0);
  const [writtenFrames, setWrittenFrames] = useState(0);
  const [recordingFps, setRecordingFps] = useState<number | null>(null);
    ...
    const timer = setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          const targetFps
            = roiPreset.fps
              ?? (effectiveRoi.width <= 1280 && effectiveRoi.height <= 720 ? 60 : 30);
          useCameraStore.getState().setDesiredStreamingFps(targetFps);
          try {
            await sendCommandWait(
              'change_streaming_frame_rate',
              [1, targetFps],
              5000,
            );
          }
          catch (fpsError) {
            console.warn('Failed to apply target streaming fps:', fpsError);
          }
  ...
  const pollSerStatus = useCallback(() => {
    statusPollRef.current = setInterval(() => {
      void sendCommandWait('get_ser_status', [], 3000).then((result) => {
        ...
        if (typeof payload.effective_fps === 'number' && payload.effective_fps > 0)
          setRecordingFps(Number(payload.effective_fps.toFixed(1)));
  ...
  const startRecording = useCallback(async () => {
    ...
    setWrittenFrames(0);
    setRecordingFps(null);

    try {
      const bitDepth = format === 'ser16' ? 16 : format === 'ser12' ? 12 : 8;
      const targetFps = roiPreset.fps ?? 30;
      if (format !== 'mp4')
        await ensureSerStreaming(targetFps);
