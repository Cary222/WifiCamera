...
export function PlanetCameraScreen({ onBack }: { onBack: () => void }) {
  ...
  const format: PlanetFormat = useMemo(() => {
    if (containerFormat === "mp4") return "mp4";
    if (bitDepth === "16-bit") return "ser16";
    if (bitDepth === "12-bit") return "ser12";
    return "ser8";
  ...
  const handleShutter = () => {
    if (!isConnected || isApplyingRoi || previewState !== "live") return;
    // 停止录制不该被倒计时延迟，只有开始拍摄才走倒计时。
    if (isRecording && !countdown.isRunning()) {
      runShutter();
      return;
    }
    if (!countdown.start()) runShutter();
  };
                ...
                {activeParamCard === "exposure" && (
                  <View className="w-full items-center py-1">
                    <LandscapeRuler
                      ...
                      onChange={(value) => {
                        if (!settingsDisabled) setExposure(value);
                      }}
                    />
                  </View>
                )}
                ...
                {activeParamCard === "gain" && (
                  <View className="w-full items-center py-1">
                    <LandscapeRuler
                      ...
                      onChange={(value) => {
                        if (!settingsDisabled) setGain(value);
                      }}
                    />
                  </View>
                )}
