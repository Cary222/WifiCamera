...
function SolveResultRow({ label, value }: { label: string; value: string }) {
  return (
    <View className="flex-row gap-3">
      <Text className="w-[58px] text-xs text-neutral-500 dark:text-white/50">
        {label}
      </Text>
...
export function NebulaCameraScreen({ onBack }: { onBack: () => void }) {
  ...
  const {
    ...
    startCountdown,
    cancel,
  } = useNebulaCapture({ exposure, gain });
  const {
    solveState,
    result: solveResult,
    solve,
  ...
  const nebulaRatio: LandscapeRatio =
    landscapeRatio === "16:9" ? "16:9" : "4:3";
  useEffect(() => {
    if (landscapeRatio === "full") setLandscapeSensorRatio("4:3");
  ...
  const {
    ...
    rotation,
    scale,
  } = useAspectRatioAnimation(nebulaRatio, 220, 12);
  ...
  const imageUrl = useMemo(() => {
    if (!newestCameraJpgUrl) return null;
  ...
  useEffect(() => {
    if (connectionStatus === "open") {
      sendCommand({
        ...
        params: [autoStretch ? 1 : 0],
        id: `APP-NEB-STRETCH-${Date.now().toString(36)}`,
      });
    }
  ...
  const shutter = () => {
    ...
    if (isCapturing || isCountdown) return cancel();
    if (captureMode === "video") {
      if (isRecording) stopRecording();
      else if (!recordingBusy) startRecording();
      return;
    }
    const options = timerEnabled ? timerPlan : {};
    if (countdownEnabled) startCountdown(countdown, options);
  ...
  return (
    ...
    >
      ...
      <CameraTopBar
        ...
        onBack={onBack}
        onTitlePress={() =>
          setSheetTarget((value) => (value === "manual" ? "tools" : "manual"))
        }
        ...
        rightContent={
          ...
          >
            <Text
              className={`text-[11px] ${isSolving ? "text-black" : isDark ? "text-white" : "text-black"}`}
            >
              {isSolving
                ? translate("nebula.resolve_in_progress")
                : translate("nebula.resolve")}
            </Text>
          </Pressable>
        }
      />
      ...
      >
        <Text
          className={`text-[11px] ${focusAssist ? "text-black" : isDark ? "text-white" : "text-black"}`}
        >
      ...
      {(status || notice) && (
        <View
          ...
          style={{ top: insets.top + 108, zIndex: 10, elevation: 10 }}
        >
          <View className="rounded-full bg-black/55 px-4 py-1.5">
            <Text className="text-xs text-white">{notice ?? status}</Text>
          </View>
        </View>
      )}
      ...
      {solveResult && (
        <View
          className="absolute inset-x-4 rounded-2xl p-4"
          style={{
            top: previewTop + 144,
        ...
        >
          <View className="mb-3 flex-row items-center justify-between">
            <Text
              className={`text-sm font-semibold ${solveResult.success ? "text-[#CBFF3C]" : "text-red-400"}`}
            >
              {solveResult.success
                ? translate("nebula.resolve_success")
                : translate("nebula.resolve_failed")}
            </Text>
            <Pressable
              onPress={dismissResult}
              className={`rounded-full px-2 py-1 active:opacity-80 ${isDark ? "bg-white/10" : "bg-neutral-100"}`}
            >
              <Text
          ...
          {solveResult.success ? (
            <View className="gap-1.5">
              <SolveResultRow
                label="赤经"
                value={formatRaCoordinate(solveResult.ra ?? Number.NaN)}
              />
              <SolveResultRow
                label="赤纬"
                value={formatDecCoordinate(solveResult.dec ?? Number.NaN)}
              />
              <SolveResultRow
                label="旋转角"
                value={
                  solveResult.orientation === null
                    ? "--"
                    : `${solveResult.orientation.toFixed(4)}°（东偏北）`
                }
              />
              <SolveResultRow
                label="像素尺度"
                value={
                  solveResult.pixelScale === null
                    ? "--"
                    : `${solveResult.pixelScale.toFixed(4)}″/px`
                }
              />
              <SolveResultRow
                label="视场"
                value={
                  solveResult.fieldWidth === null ||
                  solveResult.fieldHeight === null
                    ? "--"
                    : `${solveResult.fieldWidth.toFixed(4)}° × ${solveResult.fieldHeight.toFixed(4)}°`
                }
              />
              <SolveResultRow
                label="用时"
                value={formatSolveElapsed(solveResult.elapsedMs)}
              />
            </View>
          ) : (
            <Text
              className={`text-xs/5 ${isDark ? "text-white/80" : "text-neutral-700"}`}
            >
              {solveResult.error}
            </Text>
          )}
      ...
      {sheetOpen && (
        <View
          className="absolute inset-x-0 rounded-t-[26px] p-4"
          style={{
            ...
            <View className="gap-4">
              <View className="flex-row gap-3">
                <ToolCard
                  icon={
                    <StopwatchIcon
                      color={timerEnabled ? "#111" : isDark ? "#FFF" : "#222"}
                    />
                  }
                  label="定时拍摄"
                  active={timerEnabled}
                ...
                <ToolCard
                  icon={
                    <CountdownIcon
                      color={
                        countdownEnabled ? "#111" : isDark ? "#FFF" : "#222"
                      }
                    />
                  }
                  label="倒计时"
              ...
              <Pressable
                onPress={() => setAutoStretch((value) => !value)}
                style={{
                  backgroundColor: autoStretch
                    ? BRAND
                    : isDark
                      ? CARD_BG
                      : "#F4F4F5",
                }}
                className="h-[70px] items-center justify-center rounded-2xl"
              >
                <Text
                  className={
                    autoStretch
                ...
                >
                  ...
                  {autoStretch
                    ? translate("nebula.auto_stretch_on")
                    : translate("nebula.auto_stretch_off")}
                </Text>
              </Pressable>
            </View>
          ) : null}
              ...
              {timerEnabled && (
                <>
                  <LandscapeRuler
                    ...
                    formatValue={(value) => `${value}张`}
                    onChange={(value) =>
                      setTimerPlan((plan) => ({ ...plan, count: value }))
                    }
                  />
                  <LandscapeRuler
                    label="间隔"
                    ...
                    onChange={(value) =>
                      setTimerPlan((plan) => ({ ...plan, interval: value }))
                    }
                  />
                </>
              )}
      ...
      <CameraBottomBar
        ...
        rightButton={
          <SheetMenuIcon
            color={sheetOpen ? BRAND : isDark ? "#FFFFFF" : "#0A0B0D"}
          />
        }
