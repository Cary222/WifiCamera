...
function CoordinateDialogContent({
  ...
  const title =
    kind === "latitude"
      ? translate("deep_space.location.latitude_placeholder")
      : translate("deep_space.location.longitude_placeholder");
  ...
  return (
    <View
      ...
      testID={`deep-space-settings-${kind}-modal`}
    >
      <Text style={[styles.dialogTitle, !isDark && { color: overlay.text }]}>
        {title}
      </Text>
      <TextInput
    ...
    >
      <View style={styles.modalOverlay}>
        <View
          ...
          testID="deep-space-settings-city-modal"
        >
          <Text
            style={[styles.dialogTitle, !isDark && { color: overlay.text }]}
          >
            {translate("deep_space.location.pick_city")}
          </Text>
          <View style={styles.customCityRow}>
            ...
            <Pressable
              accessibilityLabel={translate(
                "deep_space.location.apply_custom_name",
              )}
              accessibilityRole="button"
              ...
              style={[
                styles.customCityBtn,
                !customName.trim() && { opacity: 0.5 },
              ]}
              testID="deep-space-settings-custom-city-confirm"
            >
              <Text style={styles.customCityBtnText}>
                {translate("deep_space.location.apply")}
              </Text>
            </Pressable>
          </View>
          <ScrollView style={styles.cityList}>
            {EXPANDED_OBSERVER_CITIES.map((city) => (
              <Pressable
                ...
                style={[
                  styles.cityRow,
                  !isDark && { borderBottomColor: overlay.hairline },
                ]}
              ...
              >
                <View style={featureSheetStyles.featureRowText}>
                  <Text
                    style={[
                      featureSheetStyles.featureRowLabel,
                      !isDark && { color: overlay.text },
                    ]}
                  >
                    {cityLabel(city.name)}
                  </Text>
                  <Text
                    style={[
                      featureSheetStyles.featureRowHint,
                      !isDark && { color: overlay.muted },
                    ]}
                  >{`${city.latitudeDeg.toFixed(2)}°, ${city.longitudeDeg.toFixed(2)}°`}</Text>
                </View>
          ...
          <Pressable
            accessibilityLabel={translate("deep_space.location.close")}
            accessibilityRole="button"
            onPress={onCancel}
          ...
          >
            ...
            >
              {translate("deep_space.location.close")}
            </Text>
          </Pressable>
        </View>
      </View>
