...
const TONIGHT_FIXTURE: TonightReport = {
  ...
  moon: {
    ...
    rise: "2026-08-21T14:20:00.000Z",
    set: "2026-08-22T02:05:00.000Z",
  },
  planets: [
    {
      from: "2026-08-21T11:30:00.000Z",
      key: "saturn",
      magnitude: 0.6,
      peakAltitudeDeg: 48,
      to: "2026-08-22T04:10:00.000Z",
    },
  ],
...
const mockGetCurrentPosition = jest.fn(async (_options?: unknown) => ({
  coords: { latitude: 39.9, longitude: 116.41 },
}));
const mockRequestLocationPermission = jest.fn(async () => ({
  status: "granted",
}));
const mockHeadingSubscription = { remove: jest.fn() };
let mockHeadingCallback:
  | ((heading: { trueHeading: number }) => void)
  | undefined;
...
const mockWatchPosition = jest.fn(
  async (_options?: unknown, _callback?: unknown) => ({ remove: jest.fn() }),
);
...
const mockFocusTarget = jest.fn(
  async (name: string): Promise<SelectedCelestialObject | null> => ({
    altDeg: 35.8,
    azDeg: 120.4,
    decDeg: -5.38,
    designations: [name],
...
const mockQueryTargets = jest.fn(
  async (names: string[]): Promise<TargetQueryResult[]> =>
    names.map((name) => ({
      altDeg: 42.5,
      available: true,
      azDeg: 123.4,
...
function archivedViewStateJson(): string {
  return JSON.stringify({
    version: 1,
    state: { altitudeDeg: 12.5, azimuthDeg: 271.2, fovDeg: 30.5 },
  });
}
...
function installArchivedStorage() {
  ...
  store.getString.mockImplementation((key: string) =>
    typeof map.get(key) === "string" ? (map.get(key) as string) : undefined,
  );
  store.set.mockImplementation(
    (key: string, value: string | number | boolean) => map.set(key, value),
  );
...
jest.mock("./ui/deep-space-feedback", () => ({
  showDeepSpaceFeedback: (...args: unknown[]) =>
    mockShowDeepSpaceFeedback(...args),
}));
...
jest.mock("expo-location", () => ({
  ...
  getCurrentPositionAsync: (options: unknown) =>
    mockGetCurrentPosition(options),
  ...
  watchPositionAsync: (options: unknown, callback: unknown) =>
...
jest.mock("@/lib/i18n", () => {
  const zh = jest.requireActual("@/translations/zh.json").deep_space as Record<
    string,
    unknown
  >;
  return {
    ...
    translate: (key: string, options?: Record<string, unknown>) => {
      ...
      return options
        ? value.replace(/\{\{(\w+)\}\}/g, (_match: string, name: string) =>
            String(options[name] ?? ""),
          )
        : value;
    },
...
describe("deep space map screen", () => {
  it("renders the reference-style Stellarium chrome without the custom title pill", () => {
    ...
    expect(
      screen.queryByTestId("deep-space-reference-layers"),
    ).not.toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-reference-layers-panel"),
    ).not.toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-reference-compass"),
    ).toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-reference-compass-instrument"),
    ).toBeOnTheScreen();
    expect(screen.getByTestId("deep-space-reference-time")).toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-map-title-pill"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("rotates the compass and names the bearing reported by the engine", () => {
    ...
    expect(screen.getByTestId("deep-space-reference-compass-rose")).toHaveStyle(
      { transform: [{ rotate: "-90deg" }] },
    );
    expect(
      screen.getByTestId("deep-space-reference-compass-azimuth"),
    ).toHaveTextContent("90°");
    expect(screen.getByTestId("deep-space-horizon-bearing")).toHaveTextContent(
      "东",
    );
  });
  ...
  it("keeps the compass pointing north and absolutely centered until the engine reports a bearing", () => {
    setup(<DeepSpaceMapScreen />);
    expect(
      screen.getByTestId("deep-space-reference-compass-center"),
    ).toHaveStyle({
      alignItems: "center",
      left: 0,
      position: "absolute",
  ...
  it("opens the reference-style drawer from the menu control", async () => {
    ...
    expect(screen.getByTestId("deep-space-reference-drawer")).toHaveStyle({
      paddingTop: 48,
    });
  });
  ...
  it("closes the drawer from the header close control", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-reference-drawer"),
    ).not.toBeOnTheScreen();
  });
});
...
describe("deep space glossary feature", () => {
  it("matches the reference full-screen list without region headers or placeholder glyphs", async () => {
    ...
    expect(screen.getByTestId("deep-space-glossary-panel")).toHaveStyle({
      paddingTop: 48,
    });
    expect(
      screen.getByTestId("deep-space-glossary-item-egyptian"),
    ).toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-glossary-item-arabic_ancient"),
    ).toBeOnTheScreen();
  ...
  it("switches the engine sky culture from the glossary", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-glossary-detail-chinese"),
    ).toBeOnTheScreen();
    expect(mockSetSkyCulture).toHaveBeenCalledWith("chinese");
    expect(
      screen.getByTestId("deep-space-glossary-detail-chinese"),
    ).toHaveStyle({ paddingTop: 48 });

    // 全屏详情保留原版的地区行、使用按钮和正文面板。
    expect(
      screen.getByTestId("deep-space-glossary-use-button"),
    ).toBeOnTheScreen();
    ...
    expect(mockSetSkyCulture).toHaveBeenLastCalledWith(
      "chinese",
      "CON chinese 236",
    );
    expect(
      screen.queryByTestId("deep-space-glossary-detail-chinese"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("shows a restore default floating button when a non-default sky culture is active", async () => {
    ...
    // 默认是西方文化，不显示恢复浮钮
    expect(
      screen.queryByTestId("deep-space-restore-culture-fab"),
    ).not.toBeOnTheScreen();
    ...
    // 应用后右下角出现恢复浮钮
    expect(
      screen.getByTestId("deep-space-restore-culture-fab"),
    ).toBeOnTheScreen();

    // 点击浮钮弹出确认并还原
    await user.press(screen.getByTestId("deep-space-restore-culture-fab"));
    expect(
      screen.getByTestId("deep-space-restore-culture-dialog"),
    ).toBeOnTheScreen();
...
describe("deep space settings and location features", () => {
  it("opens the official settings root before selecting a location", async () => {
    ...
    expect(screen.getByTestId("deep-space-settings-panel")).toHaveStyle({
      alignSelf: "stretch",
    });
    expect(screen.getByTestId("deep-space-settings-panel")).toHaveStyle({
      paddingTop: 48,
    ...
    expect(
      screen.queryByTestId("deep-space-settings-location-上海"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("uses automatic location from the official settings location page", async () => {
    ...
    await user.press(
      screen.getByTestId("deep-space-settings-auto-location-toggle"),
    );
  ...
  it("updates manual latitude and longitude from coordinate input dialogs", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-settings-latitude-modal"),
    ).toBeOnTheScreen();
    await user.clear(screen.getByTestId("deep-space-settings-latitude-input"));
    await user.type(
      screen.getByTestId("deep-space-settings-latitude-input"),
      "24.5",
    );
    await user.press(
      screen.getByTestId("deep-space-settings-latitude-confirm"),
    );
    ...
    expect(
      screen.getByTestId("deep-space-settings-longitude-modal"),
    ).toBeOnTheScreen();
    await user.clear(screen.getByTestId("deep-space-settings-longitude-input"));
    await user.type(
      screen.getByTestId("deep-space-settings-longitude-input"),
      "118.5",
    );
    await user.press(
      screen.getByTestId("deep-space-settings-longitude-confirm"),
    );
  ...
  it("selects city and updates observer from city picker modal", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-settings-city-modal"),
    ).toBeOnTheScreen();
...
describe("deep space advanced settings and reset features", () => {
  it("navigates to the official advanced settings subpage and back", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-settings-advanced-panel"),
    ).toBeOnTheScreen();
  ...
  it("retains star-map controls when fullscreen is enabled, reveals exit button on corner tap and restores standard mode on exit", async () => {
    ...
    await user.press(screen.getByTestId("deep-space-settings-advanced-entry"));
    await user.press(
      screen.getByTestId("deep-space-settings-fullscreen-toggle"),
    );
    await user.press(screen.getByLabelText(translate("deep_space.back")));

    expect(
      screen.queryByTestId("deep-space-settings-advanced-panel"),
    ).not.toBeOnTheScreen();
    // In full-screen mode, star-map own controls remain accessible
    expect(screen.getByTestId("deep-space-reference-menu")).toBeOnTheScreen();
    expect(screen.getByTestId("deep-space-reference-search")).toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-grid-quick-toggle"),
    ).toBeOnTheScreen();
    // Exit button is not shown permanently to avoid cluttering the view.
    expect(
      screen.queryByTestId("deep-space-exit-fullscreen"),
    ).not.toBeOnTheScreen();

    // Tapping the top-right corner reveals the exit button.
    await user.press(
      screen.getByTestId("deep-space-fullscreen-corner-trigger"),
    );
  ...
  it("shows the official reset settings confirmation dialog and cancels or confirms", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-settings-reset-dialog"),
    ).toBeOnTheScreen();
    ...
    expect(
      screen.queryByTestId("deep-space-settings-reset-dialog"),
    ).not.toBeOnTheScreen();
  ...
  it("opens the reference calendar without the legacy time-shift controls", async () => {
    ...
    expect(screen.getByTestId("deep-space-calendar-panel")).toHaveStyle({
      paddingTop: 48,
    });
    expect(
      screen.queryByTestId("deep-space-calendar-forward-day"),
    ).not.toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-calendar-back-hour"),
    ).not.toBeOnTheScreen();
  });
});
...
describe("deep space calendar panel", () => {
  it("renders tonight ephemeris computed for the selected observer city", async () => {
    ...
    expect(mockComputeTonight).toHaveBeenCalledWith(expect.any(Date), {
      latitudeDeg: 39.9,
      longitudeDeg: 116.41,
    });
    expect(
      screen.getByTestId("deep-space-calendar-chart-saturn"),
    ).toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-calendar-chart-saturn-ring"),
    ).toBeOnTheScreen();
  ...
  it("switches to the events tab and groups what the engine returned", async () => {
    ...
    expect(mockComputeEvents).toHaveBeenCalledWith(expect.any(Date), 60, {
      latitudeDeg: 39.9,
      longitudeDeg: 116.41,
    });
  ...
  it("recomputes the calendar when the observer city changes", async () => {
    ...
    await user.press(screen.getByText("日历"));
    await screen.findByTestId("deep-space-calendar-tonight");
    expect(mockComputeTonight).toHaveBeenLastCalledWith(expect.any(Date), {
      latitudeDeg: 31.23,
      longitudeDeg: 121.47,
    });
  });
  ...
  it("shows a safe empty state when the night window is unknown", async () => {
    ...
    await screen.findByTestId("deep-space-calendar-tonight");
    expect(
      screen.queryByTestId("deep-space-calendar-chart"),
    ).not.toBeOnTheScreen();
  ...
  it("labels each event with its name and full local time", async () => {
    ...
    expect(
      screen.getByText(/^八月 28, \d{2}:\d{2} GMT[+-]\d{2}:\d{2}$/),
    ).toBeOnTheScreen();
  });
  ...
  it("offers a retry when the engine fails to compute the calendar", async () => {
    ...
    await user.press(screen.getByText("日历"));
    expect(
      await screen.findByTestId("deep-space-calendar-error"),
    ).toBeOnTheScreen();
...
describe("deep space 3x2 quick controls", () => {
  it("opens the reference 3×2 Stellarium overlay panel and applies its grouped controls", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-reference-compass"),
    ).not.toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-reference-time"),
    ).not.toBeOnTheScreen();

    await user.press(screen.getByTestId("deep-space-grid-quick-grid-lines"));
    expect(mockSetGridLines).toHaveBeenCalledWith({
      azimuthal: true,
      equatorial_jnow: true,
    });
    await user.press(screen.getByTestId("deep-space-grid-quick-night-mode"));
    expect(
      screen.getByTestId("deep-space-night-mode-overlay"),
    ).toBeOnTheScreen();
  });
  ...
  it("toggles all labels on quick button press and keeps the landscape enabled", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-grid-quick-landscape").props
...
describe("deep space quick detail sheets", () => {
  it("opens secondary detail settings sheet on long pressing quick controls", async () => {
    ...
    await user.longPress(
      screen.getByTestId("deep-space-grid-quick-grid-lines"),
    );
    expect(
      screen.getByTestId("deep-space-quick-detail-sheet"),
    ).toBeOnTheScreen();
    ...
    // 细粒度切换黄道线与子午线
    await user.press(
      screen.getByTestId("deep-space-quick-detail-toggle-ecliptic"),
    );
    expect(mockSetGridLines).toHaveBeenLastCalledWith({ ecliptic: true });
    await user.press(
      screen.getByTestId("deep-space-quick-detail-toggle-meridian"),
    );
    ...
    expect(
      screen.queryByTestId("deep-space-quick-detail-sheet"),
    ).not.toBeOnTheScreen();

    // 长按星座按钮
    await user.longPress(
      screen.getByTestId("deep-space-grid-quick-constellation"),
    );
    expect(
      screen.getByTestId("deep-space-quick-detail-sheet"),
    ).toBeOnTheScreen();
  ...
  it("sends alternating sky-layer commands when the same switch is tapped twice in a row", async () => {
    ...
    await user.press(
      screen.getByTestId("deep-space-quick-detail-toggle-constellationLabels"),
    );
    await user.press(
      screen.getByTestId("deep-space-quick-detail-toggle-constellationLabels"),
    );

    expect(mockSetSkyLayers).toHaveBeenNthCalledWith(2, {
      constellationLabels: false,
    });
    expect(mockSetSkyLayers).toHaveBeenNthCalledWith(3, {
      constellationLabels: true,
    });
  });
  ...
  it("sends the two advanced constellation display switches to the engine", async () => {
    ...
    await user.press(
      screen.getByTestId(
        "deep-space-quick-detail-toggle-constellationBoundaries",
      ),
    );
    await user.press(
      screen.getByTestId(
        "deep-space-quick-detail-toggle-constellationOnlyPointed",
      ),
    );
  ...
  it("starts atmosphere and fog disabled and allows each switch to be enabled independently", async () => {
    const { user } = setup(<DeepSpaceMapScreen />);
    expect(mockSetSkyLayers).toHaveBeenCalledWith(
      expect.objectContaining({ atmosphere: false }),
    );
    ...
    await user.press(screen.getByTestId("deep-space-grid-quick-toggle"));
    await user.longPress(
      screen.getByTestId("deep-space-grid-quick-atmosphere"),
    );
    ...
    expect(
      screen.getByTestId("deep-space-quick-detail-toggle-atmosphere").props
        .accessibilityState.checked,
    ).toBe(false);
    expect(
      screen.getByTestId("deep-space-quick-detail-toggle-fog").props
        .accessibilityState.checked,
    ).toBe(false);
    ...
    await user.press(
      screen.getByTestId("deep-space-quick-detail-toggle-atmosphere"),
    );
...
describe("deep space labels detail sheet", () => {
  it("opens Stellarium labels detail sheet on long press with 4 sliders and reset button", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-quick-detail-close"),
    ).toBeOnTheScreen();
    expect(screen.queryByText("‹")).not.toBeOnTheScreen();
    ...
    expect(
      screen.getByTestId("deep-space-labels-reset-button"),
    ).toBeOnTheScreen();
    expect(screen.getByText("重置数值")).toBeOnTheScreen();
    expect(screen.queryByText("星座标签")).not.toBeOnTheScreen();

    fireEvent(
      screen.getByTestId("deep-space-label-slider-stars"),
      "accessibilityAction",
      {
  ...
  it("resets grid lines from quick detail reset button", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-quick-detail-reset"),
    ).toBeOnTheScreen();
  ...
  it("resets enabled atmosphere and fog to off without changing the other saved layers", async () => {
    ...
    expect(
      storedJson(map, STORAGE_KEYS.DEEP_SPACE_VIEW_PREFERENCES),
    ).toMatchObject({
...
describe("deep space interactive time control", () => {
  it("opens time control bar when tapping time capsule", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-time-slider-sheet"),
    ).toBeOnTheScreen();
  ...
  it("returns to now and closes time control bar", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-time-slider-sheet"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("advances custom time at the selected preview speed while playback is active", () => {
    ...
    try {
      ...
      expect(
        screen.getByTestId("deep-space-time-playback-toggle").props
...
describe("deep space time state ownership", () => {
  ...
  it("keeps playback and speed after closing and reopening the existing panel", () => {
    ...
    expect(
      screen.getByTestId("deep-space-time-speed-60").props.accessibilityState
        .selected,
    ).toBe(true);
  });
  ...
  it("offers reverse speeds and explicit zero pause without replacing the panel", () => {
    ...
    for (const speed of [-60, -1, 0, 1, 10, 60, 600]) {
      expect(
        screen.getByTestId(`deep-space-time-speed-${speed}`),
      ).toBeOnTheScreen();
    }
    expect(screen.getByTestId("deep-space-time-playback-controls")).toHaveStyle(
      { flexWrap: "wrap" },
    );
    fireEvent.press(screen.getByTestId("deep-space-time-speed--60"));
    fireEvent.press(screen.getByTestId("deep-space-time-playback-toggle"));
    act(() => jest.advanceTimersByTime(1000));
    expect(mockSetTime).toHaveBeenLastCalledWith(
...
function expectRestoreOrder() {
  const commands = [
    ...
    mockSetGridLines,
    mockSetBrightness,
    mockSetMagnitudeLimit,
    mockRestoreView,
  ];
  const order = commands.map((command) =>
    command.mock.invocationCallOrder.at(-1),
  );
...
describe("deep space observer context restoration", () => {
  ...
  it("restores the selected last-view date on the initial engine ready", () => {
    ...
    try {
      ...
      fireEvent.press(screen.getByTestId("deep-space-reference-time"));
      expect(
        screen.getByTestId("deep-space-time-date-value"),
      ).toHaveTextContent("2025年3月4日");
    } finally {
      (storage.getString as jest.Mock).mockReset();
    }
  });
...
describe("deep space observation tools", () => {
  it("keeps telescope and field-of-view tools separate from the left-bottom grid controls", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-tools-azimuthal"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("removes the field frame from the observation tools", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-fov-overlay"),
    ).not.toBeOnTheScreen();
  });
...
describe("deep space celestial search", () => {
  it("focuses a typed celestial target through the real engine bridge", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-reference-search-sheet"),
    ).toBeOnTheScreen();
    ...
    await waitFor(() =>
      expect(mockFocusTarget).toHaveBeenLastCalledWith("M 42"),
    );
    await waitFor(() =>
      expect(
        screen.queryByTestId("deep-space-reference-search-sheet"),
      ).not.toBeOnTheScreen(),
    );
  });
  ...
  it("shows real-time suggestions and navigates to target when tapped", async () => {
    ...
    expect(screen.getByTestId("deep-space-map-search-input")).toHaveProp(
      "value",
      "",
    );
    ...
    await waitFor(() =>
      expect(mockFocusTarget).toHaveBeenLastCalledWith("Vega"),
    );
  });
  ...
  it("filters celestial suggestions by category tabs", async () => {
    ...
    await user.press(
      screen.getByTestId("deep-space-search-category-solar_system"),
    );

    expect(
      screen.getByTestId("deep-space-search-item-NAME Mars"),
    ).toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-search-item-Vega"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("moves to a Stellarium-style RA/Dec coordinate query without name lookup", async () => {
    ...
    await user.press(screen.getByTestId("deep-space-reference-search"));
    await user.type(
      screen.getByTestId("deep-space-map-search-input"),
      "6h45m7s 16d43m29s",
    );
    await user.press(screen.getByTestId("deep-space-map-search-submit"));

    expect(mockGotoRaDec).toHaveBeenLastCalledWith(
      101.27916666666667,
      16.72472222222222,
    );
  ...
  it("keeps the star map available when the engine cannot find a target", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("deep-space-map-search-error"),
      ).toHaveTextContent("未找到该天体，请改用标准名称或编号"),
    );
    expect(screen.getByTestId("deep-space-map-shell")).toBeOnTheScreen();
  });
  ...
  it("does not misreport an unrelated engine command error as a missing target", async () => {
    ...
    expect(screen.getByTestId("deep-space-map-shell")).toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-map-search-error"),
    ).not.toBeOnTheScreen();
    expect(mockShowDeepSpaceFeedback).toHaveBeenCalledWith(
...
describe("search selection lifecycle integration", () => {
  ...
  it("shows the resolved target details and records the canonical ID in history", async () => {
    ...
    await user.press(
      screen.getByTestId("deep-space-search-item-CON western Boo"),
    );
    expect(
      await screen.findByTestId("deep-space-object-info-sheet"),
    ).toHaveTextContent(/牧夫座/);
    ...
    expect(
      screen.getByTestId("deep-space-search-recent-CON western Boo"),
    ).toBeOnTheScreen();
  });
  ...
  it("ignores a cancelled result that arrives after the search sheet has reopened", async () => {
    ...
    mockFocusTarget.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          finish = resolve;
        }),
  ...
  it("does not let an earlier success overwrite the most recently selected object", async () => {
    ...
    mockFocusTarget.mockResolvedValueOnce({
      ...bootes,
      id: "CON western Leo",
      name: "狮子座",
      englishName: "Leo",
    ...
    await user.press(
      screen.getByTestId("deep-space-search-item-CON western Leo"),
    );
    await act(async () => finish(bootes));
    expect(
      await screen.findByTestId("deep-space-object-info-sheet"),
    ).toHaveTextContent(/狮子座/);
    expect(screen.queryByText("牧夫座")).not.toBeOnTheScreen();
  });
});

describe("search reason propagation", () => {
  it.each(["loading", "culture_mismatch", "missing_data"] as const)(
    "keeps focus %s distinct and retries only when requested",
    async (reason) => {
      mockFocusTarget.mockRejectedValueOnce(
        new Error(`FOCUS_UNAVAILABLE:${reason}`),
      );
      setup(<DeepSpaceMapScreen />);
      ...
      fireEvent.press(
        screen.getByTestId("deep-space-search-item-NAME Jupiter"),
      );
      expect(
        await screen.findByTestId("deep-space-map-search-error"),
      ).toHaveTextContent(translate(`deep_space.search_${reason}`));
      expect(mockSetSkyCulture).toHaveBeenLastCalledWith("western");
  ...
  it.each(["timeout", "Stellarium bridge reloaded.", "loading"])(
    "does not present %s as a missing object",
    async (message) => {
      ...
      expect(
        await screen.findByTestId("deep-space-map-search-error"),
      ).toHaveTextContent(translate("deep_space.search_failed"));
    },
  );
});
...
describe("bounded loading metrics refresh", () => {
  const loading = (ids: string[]): Promise<TargetQueryResult[]> =>
    Promise.resolve(
      ids.map((id) => ({ id, available: false, reason: "loading" })),
    );
  beforeEach(() => jest.useFakeTimers());
  afterEach(() => {
    cleanup();
    mockQueryTargets.mockImplementation(async (names) =>
      names.map((id) => ({ id, available: true, altDeg: 42.5, vmag: 1.2 })),
    );
    jest.useRealTimers();
  });
  ...
  it("queries a new visible group while the old request is pending and ignores the old response", async () => {
    ...
    mockQueryTargets.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          finishOld = resolve;
        }),
    ...
    expect(mockQueryTargets).toHaveBeenLastCalledWith([
      "NAME Jupiter",
      "NAME Io",
      "NAME Europa",
      "NAME Ganymede",
    ...
    await act(async () =>
      finishOld([{ id: "NAME Jupiter", available: false, reason: "loading" }]),
    );
    await act(async () => jest.advanceTimersByTime(3000));
    expect(mockQueryTargets).toHaveBeenCalledTimes(2);
    expect(
      screen.queryByTestId("deep-space-search-retry"),
    ).not.toBeOnTheScreen();
  });
});
...
describe("manual search metrics refresh", () => {
  ...
  it("keeps the visible snapshot until manual refresh when automatic observer coordinates change", async () => {
    ...
    const calls = mockQueryTargets.mock.calls.length;
    await act(async () =>
      moveObserver({ coords: { latitude: 5, longitude: 10 } }),
    );
...
describe("manual visible snapshot lifecycle", () => {
  ...
  it("reuses the visible snapshot after visiting a search result instead of querying it again", async () => {
    ...
    expect(
      screen
        .getAllByTestId(/^deep-space-popular-/)
        .map((row) => row.props.testID),
    ).toEqual(["deep-space-popular-NAME Sun", "deep-space-popular-NAME Moon"]);
  });
  ...
  it("keeps the old list during a manual refresh, rejects duplicate taps and replaces it only on completion", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-popular-NAME Moon"),
    ).toBeOnTheScreen();
    fireEvent.press(screen.getByTestId("deep-space-visible-refresh"));
    expect(mockQueryTargets).toHaveBeenCalledTimes(2);
    await act(async () =>
    ...
    expect(
      screen.queryByTestId("deep-space-popular-NAME Sun"),
    ).not.toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-popular-NAME Moon"),
    ).toBeOnTheScreen();
  ...
  it("ignores a manual refresh that resolves after closing and reopening search", async () => {
    ...
    fireEvent.press(screen.getByTestId("deep-space-search-backdrop"));
    mockQueryTargets.mockResolvedValueOnce([
      { id: "NAME Jupiter", available: true, altDeg: 75 },
    ]);
    fireEvent.press(screen.getByTestId("deep-space-reference-search"));
    await flushSearchRequests();
    fireEvent.press(screen.getByTestId("deep-space-search-tab-visible"));
    expect(mockQueryTargets).toHaveBeenCalledTimes(3);
    await act(async () =>
      finishOld([{ id: "NAME Sun", available: true, altDeg: 80 }]),
    );
    expect(
      screen.getByTestId("deep-space-popular-NAME Jupiter"),
    ).toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-popular-NAME Sun"),
    ).not.toBeOnTheScreen();
...
describe("detail observing-context integration", () => {
  const object: SelectedCelestialObject = {
    id: "HIP 32349",
    catalogId: "Sirius",
    name: "天狼星",
    englishName: "Sirius",
    designations: ["HIP 32349"],
    raHours: 6.75,
    decDeg: -16.7,
    altDeg: 10,
    azDeg: 20,
  ...
  it("refreshes paused details after ready without focus or duplicate history and stops after close", async () => {
    ...
    const historyWrites = (storage.set as jest.Mock).mock.calls.filter(
      ([key]) => key === STORAGE_KEYS.DEEP_SPACE_RECENT_OBJECTS,
    ).length;
    const focusCalls = mockFocusTarget.mock.calls.length;
    ...
    expect(
      screen.getByTestId("deep-space-search-recent-Sirius"),
    ).toBeOnTheScreen();
  });
});
...
describe("deep space air quality integration", () => {
  it("defaults air quality to Bortle 1 and cycles it through the existing stepper", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-quick-detail-stepper-air-quality"),
    ).toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-quick-detail-stepper-air-quality-value"),
    ).toHaveTextContent("Bortle 1 · 极佳暗空");

    await user.press(
      screen.getByTestId("deep-space-quick-detail-stepper-air-quality-next"),
    );
    expect(mockSetEnvironment).toHaveBeenLastCalledWith({ bortleIndex: 2 });

    await user.press(
      screen.getByTestId("deep-space-quick-detail-stepper-air-quality-prev"),
    );
...
describe("deep space landscape and environment integration", () => {
  ...
  it("opens the same quick detail sheet as the other controls on long pressing the quick landscape control", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-quick-detail-toggle-landscape"),
    ).toBeOnTheScreen();
  });
  ...
  it("sends environment changes to the engine from the quick detail sheet", async () => {
    ...
    await user.press(
      screen.getByTestId("deep-space-quick-detail-toggle-cardinals"),
    );
  ...
  it("steps forward through the landscapes without leaving the sheet", async () => {
    ...
    expect(
      screen.getByTestId(
        "deep-space-quick-detail-stepper-landscape-library-value",
      ),
    ).toHaveTextContent("盖兰");

    await user.press(
      screen.getByTestId(
        "deep-space-quick-detail-stepper-landscape-library-next",
      ),
    );
  ...
  it("wraps around to the last landscape when stepping backwards from the first", async () => {
    ...
    await user.press(
      screen.getByTestId(
        "deep-space-quick-detail-stepper-landscape-library-prev",
      ),
    );
...
describe("deep space celestial object info integration", () => {
  ...
  it("surfaces a selected object in recent search and lets the user revisit it", async () => {
    ...
    await user.press(screen.getByText(translate("deep_space.search_recent")));
    await user.press(
      screen.getByTestId("deep-space-search-recent-NAME Great Orion Nebula"),
    );

    await waitFor(() =>
      expect(mockFocusTarget).toHaveBeenLastCalledWith(
  ...
  it("does not offer unsupported telescope pointing in the selected object sheet", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-object-goto-btn"),
    ).not.toBeOnTheScreen();
    ...
    expect(
      screen.queryByTestId("deep-space-tools-panel"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("dismisses info sheet and clears engine selection on close button tap", async () => {
    ...
    expect(
      screen.queryByTestId("deep-space-object-info-sheet"),
    ).not.toBeOnTheScreen();
...
describe("deep space compass and azimuth controls", () => {
  it("keeps top controls clean with only menu and search buttons", () => {
    ...
    expect(screen.getByTestId("deep-space-reference-search")).toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-compass-follow"),
    ).not.toBeOnTheScreen();
  });
  ...
  it("opens azimuth input dialog on tapping angle readout and rotates sky bearing", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-reference-compass-azimuth-btn"),
    ).toBeOnTheScreen();
    await user.press(
      screen.getByTestId("deep-space-reference-compass-azimuth-btn"),
    );
    ...
    expect(
      screen.getByTestId("deep-space-azimuth-input-dialog"),
    ).toBeOnTheScreen();
    ...
    expect(
      screen.queryByTestId("deep-space-azimuth-input-dialog"),
    ).not.toBeOnTheScreen();
    expect(
      screen.getByTestId("deep-space-reference-compass-azimuth"),
...
describe("deep space view preference archive", () => {
  it.each([true, false])(
    "keeps explicitly saved atmosphere and fog values %s when reopening",
    async (enabled) => {
      const map = installArchivedStorage();
      const archived = JSON.parse(archivedPreferencesJson());
      ...
      expect(mockSetSkyLayers).toHaveBeenLastCalledWith(
        expect.objectContaining({ atmosphere: enabled }),
      );
      expect(mockSetEnvironment).toHaveBeenLastCalledWith(
        expect.objectContaining({ fog: enabled }),
      );
  ...
  it("restores the archived switches and camera view when the map is reopened", () => {
    ...
    expect(mockSetLandscape).toHaveBeenLastCalledWith("ocean");
    expect(mockSetGridLines).toHaveBeenLastCalledWith(
      expect.objectContaining({ meridian: true }),
    );
    expect(mockSetEnvironment).toHaveBeenLastCalledWith(
    ...
    expect(mockRestoreView).toHaveBeenLastCalledWith({
      altitudeDeg: 12.5,
      azimuthDeg: 271.2,
      fovDeg: 30.5,
    });
  ...
  it("keeps the changed switches and the camera angle across a close and reopen", async () => {
    ...
    expect(mockSetLandscape).toHaveBeenLastCalledWith("winterfield");
    expect(mockSetGridLines).toHaveBeenLastCalledWith(
      expect.objectContaining({ azimuthal: true, equatorial_jnow: true }),
    );
    expect(mockSetSkyLayers).toHaveBeenLastCalledWith(
      expect.objectContaining({
        dsoLabels: false,
...
describe("deep space camera view archive", () => {
  ...
  it("never archives a camera view outside the engine range", () => {
    ...
    act(() =>
      mockOnViewStateChange?.({ altitudeDeg: 10, azimuthDeg: 400, fovDeg: 40 }),
    );
  ...
  it("drops the archive on reset and reopens the map on its defaults", async () => {
    const map = installArchivedStorage();
    map.set(
      STORAGE_KEYS.DEEP_SPACE_VIEW_PREFERENCES,
      archivedPreferencesJson(),
    );
    map.set(STORAGE_KEYS.DEEP_SPACE_VIEW_STATE, archivedViewStateJson());
    const { unmount, user } = setup(<DeepSpaceMapScreen />);
    expect(
    ...
    expect(map.has(STORAGE_KEYS.DEEP_SPACE_VIEW_STATE)).toBe(false);
    expect(
      screen.queryByTestId("deep-space-night-mode-overlay"),
    ).not.toBeOnTheScreen();
    expect(
      screen.queryByTestId("deep-space-restore-culture-fab"),
    ).not.toBeOnTheScreen();
    ...
    expect(mockSetGridLines).toHaveBeenLastCalledWith(
      expect.objectContaining({ meridian: false }),
    );
    expect(mockSetSkyLayers).toHaveBeenLastCalledWith(
      expect.objectContaining({ atmosphere: false }),
    );
...
describe("deep space centering releases the sensor", () => {
  ...
  it("stops the sensor before locking the selected object and ignores what arrives late", async () => {
    ...
    expect(
      screen.getByTestId("deep-space-object-center-btn"),
    ).toBeOnTheScreen();
    ...
    expect(mockPointAndLock).toHaveBeenLastCalledWith(
      "NAME Great Orion Nebula",
    );
    expect(mockHeadingSubscription.remove).toHaveBeenCalledTimes(
      baselines.removals + 1,
    );
    // The details sheet stays open and nothing else moves the camera.
    expect(
      screen.getByTestId("deep-space-object-info-sheet"),
    ).toBeOnTheScreen();
    ...
    fireEvent.press(screen.getByText("设置"));
    expect(
      screen.getByTestId("deep-space-settings-sensor-toggle").props
        .accessibilityState.checked,
    ).toBe(false);
  });
